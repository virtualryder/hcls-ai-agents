"""
Structural tests for the manufacturing_batch_review.asl.json Step Functions definition.
...
ASL_PATH = os.path.join(
    ...
    "..",
    "stepfunctions",
    "manufacturing_batch_review.asl.json",
)
...
def test_required_states_present(asl):
    states = asl["States"]
    for name in ("Assemble", "Draft", "Check", "RouteChoice", "QAReviewGate", "Finalize"):
        assert name in states, f"Missing state: {name}"


def test_hitl_gate_uses_wait_for_task_token(asl):
    hitl = asl["States"]["QAReviewGate"]
...
def test_route_choice_has_hitl_branch(asl):
    choices = asl["States"]["RouteChoice"]["Choices"]
    targets = {c["Next"] for c in choices}
    assert "QAReviewGate" in targets
