...
| "Our data can't leave our environment" | "It doesn't. Inference runs in-account on Amazon Bedrock. Your PHI never leaves your VPC. The MCP gateway short-circuits any path from agent to external API." |
...
- **All eight agents are built to flagship depth** and run end-to-end with no API key (`EXTRACT_MODE=demo`) — useful for live working sessions before any AWS account exists.
...
- **Proof of rigor:** 452 automated tests pass across platform, governance, agents, and AWS-native rebuilds — cite this when Quality or Security asks "how do we know it behaves?"
