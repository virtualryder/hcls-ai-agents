...
|---|---|---|
| **Private-connectivity AWS (Bedrock)** | Inference and storage run in the customer's AWS account; Bedrock is a regional AWS service reached only over AWS PrivateLink, and PHI is masked before any model call (no egress to external AI APIs). Not an absolute "data never leaves the VPC" guarantee — see the shared-responsibility model. | Any function involving identified PHI, HIPAA data, or confidential submission content. Default recommendation for regulatory, PV, and clinical functions. |
...
- **Proof of rigor:** 575 automated tests pass across platform, governance, agents, and AWS-native rebuilds — cite this when Quality or Security asks "how do we know it behaves?"
