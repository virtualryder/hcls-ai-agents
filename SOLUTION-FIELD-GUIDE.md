...
|---|---|---|
| **Private-connectivity AWS (Bedrock)** | Inference and storage run in the customer's AWS account; Bedrock is a regional AWS service reached only over AWS PrivateLink, and PHI is masked at the audit and model-output boundaries (input filterable by Bedrock Guardrails, not blanket pre-scrubbed, no egress to external AI APIs). Not an absolute "data never leaves the VPC" guarantee — see the shared-responsibility model. | Any function involving identified PHI, HIPAA data, or confidential submission content. Default recommendation for regulatory, PV, and clinical functions. |
...
| "Our data can't leave our environment" | "There is no egress to the public internet or to an external AI API. Inference runs on HIPAA-eligible Amazon Bedrock — a regional AWS service your VPC reaches only over AWS PrivateLink — under your AWS BAA. The MCP gateway short-circuits any path from agent to an external API. HIPAA-eligible is not HIPAA-compliant; your BAA, configuration, and controls complete the picture." |
...
- **Proof of rigor:** 583 automated tests pass across platform, governance, agents, and AWS-native rebuilds — cite this when Quality or Security asks "how do we know it behaves?"
