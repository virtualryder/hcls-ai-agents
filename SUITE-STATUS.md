...
| Automated tests passing | **536** (platform 50 · governance 7 · agents 277 · native 185), verified in one command via `make test`; **all 9 golden paths also deployed + run end-to-end in a clean AWS account** |
...
- **External-review remediation (P0)** — an independent review (58/100) found gaps between the control
  ...
  **immutable fail-closed audit** (conditional `PutItem`, no Update/BatchWrite IAM); **customer IdP federation** (Cognito
  ...
  completed (internal ALB + `/ping`), SAM golden path canonical; **CI fail-closed**. Suite **492 → 503 → 536** (see `MATURITY.yaml`; run `make test`).
...
- **Security & deployability deepening (SLG-parity pass)** — closed the gaps where SLG-AI-Agents went deeper, so HCLS now exceeds it on every axis:
  ...
  - **One-command test harness:** `make test` / `scripts/run_all_tests.sh` runs all **536** tests across 20 suites and prints one total; root `conftest.py` + `pytest.ini`.
...
- **Cited professional GTM deck system + GTM citation spine** — brought GTM collateral to (and past) the EDU/SLG standard:
  - **`decks/` — 12 generated decks (10 per-agent incl. 2 roadmap + executive overview + CIO board deck).** `build-agent-decks.js` (pptxgenjs) drives 8 per-agent professional decks (issue → cost of doing nothing → governed pipeline → AWS architecture & traffic flow → proof/deploy) + the executive overview from one `AGENTS` content array; `build-cio-deck.js` drives the **CIO / CISO / Director-of-Architecture adoption-review** board deck (verdict, six gateway controls, shortfalls, 3-part shared-responsibility matrix, phased path, go/no-go). neutral dark palette + speaker notes on every slide. `make decks` builds + recompresses; `make decks-pdf` exports print-ready PDF leave-behinds.
...
- **Deployable-in-a-new-account hardening** — closed the gaps that blocked a clean first deploy and made both deployment paths real:
  ...
  - **Two MCP gateway modes, one policy** — new `gateway-portable.yaml` (API Gateway HTTP API + Cognito JWT authorizer, **any commercial or GovCloud Region**) alongside the fixed `agentcore-gateway.yaml` (now references real connector ARNs, all 12 targets). Pick with `GatewayMode`.
