...
A deterministic Safe-Harbor pass masks the structured identifier families — patient/case IDs, dates more specific than a year, contact details — before any content enters an LLM prompt. Free-text patient names (Safe Harbor #1) are caught by the ML NER pass (Amazon Comprehend Medical), which is mandatory and fails closed when real-data mode (`ALLOW_REAL_DATA`) is enabled — real PHI never falls back to the regex-only pass. Even if an adversary extracted the model's context window, they would receive masked text, not identified PHI. The mapping table is held under a customer-managed KMS key.
