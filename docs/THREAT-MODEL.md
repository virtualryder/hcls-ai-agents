...
| T4 | **Information disclosure (PHI egress)** | Patient data leaks to the model, logs, or a third party | PHI masking **before** any model/audit write; in-account Bedrock via VPC endpoint; Guardrails PII filters; no data egress | `phi.py`, `llm_factory.py`, `security.yaml` |
