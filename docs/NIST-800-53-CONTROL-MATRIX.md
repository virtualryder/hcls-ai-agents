...
| **SC — System & Comms Protection** | SC-7, SC-8, SC-12, SC-13, SC-28 | Per-customer VPC + edge (CloudFront/WAF); TLS 1.3; KMS CMK per data class; private-connectivity Bedrock via VPC endpoints (PrivateOnly mode = no NAT route) | `network.yaml`, `edge.yaml`, `security.yaml` | Reference |
...
| **SA — System & Services Acq** | SA-11, SA-15 | 583 automated tests incl. governance/red-team; CI gates every change | `make test`, `.github/workflows/ci.yml` | Reference |
