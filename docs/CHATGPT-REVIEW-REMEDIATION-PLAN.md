...
| F6 IdP federation | ◑ specified; needs AWS | `UserPoolIdentityProvider` design captured; cfn-lint `W2001` confirms the param is currently unwired |
| F7 network isolation | ◑ specified; needs AWS | VPC-attach + Bedrock/S3/DDB endpoints + restricted egress, OR correct the claim — pending sandbox account |
| F8 container path | ◑ specified; needs AWS | golden-path SAM is canonical; container path marked reference-only pending ALB/health/params |
| P1 / P2 | _backlog_ | per the tables above |

**Verification:** full suite **503 tests green** in one command (was 492; +7 finalize approval-integrity, +4 connector identity); all templates cfn-lint clean of E-level findings.
