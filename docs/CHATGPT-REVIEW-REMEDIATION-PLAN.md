...
## Round 2 — second external review (58 → 68): triage & status

All 11 findings validated against the code; in-sandbox-fixable items implemented and tested
(suite **503 → 518**), the rest authored and honestly scoped. Branch: `remediation/round-2`.

| # | Finding | Verdict | What was done | Evidence |
|---|---|---|---|---|
| 1 | Approval replay protection is process-local (`_USED_JTIS` set) | ✅ Confirmed → **fixed** | Durable single-use registry: jti claimed by atomic conditional `PutItem(attribute_not_exists(jti))` in `APPROVAL_CONSUMPTION_TABLE`; in-memory only as demo fallback | `approvals.py` `_consume_jti`/`_ddb_claim`; `test_approval_consumption.py` (4) incl. cross-environment replay |
| 2 | No deployed reviewer service (minter is a stand-in) | ✅ Confirmed → **scaffolded** (needs live) | Authenticated reviewer service: JWT-claims identity, role authorization, **server-side bound-token minting**, SoD, SendTaskSuccess/Failure; SAM template (JWT authorizer) | `services/reviewer-service/` + `test_reviewer_service.py` (5). UI + live federated run pending |
| 3 | Golden path doesn't wire the safety connector | ✅ Confirmed → **fixed** | `SAFETY_CONNECTOR_FUNCTION` wired to `FinalizeFn` (+ invoke IAM); connector **mandatory under `STRICT_APPROVAL`**; `REVIEW_TABLE` env added so the review queue actually persists | golden-path `template.yaml`; `finalize._submit_report`; `test_finalize` strict-guard test |
| 4 | Audit record < documentation; tool runs before audit | ✅ Confirmed → **fixed** | Connector writes an immutable **INTENT** row before the action and a **COMMITTED** row (with the system-of-record response) after — full evidence schema (identity, agent, args_hash, approval jti, reviewer, model/prompt version, lineage); idempotency key; fail-closed; orphan **reconciliation** | `_shared/connector/handler.py`, `reconcile.py`; `test_connector_audit.py` (5) |
| 5 | Pending-review persistence fails open | ✅ Confirmed → **fixed** | `hitl_notify` raises (workflow routes to its catch/exception path) when the review record can't be durably created; duplicate = idempotent | `hitl_notify.py` |
| 6 | "No public egress by default" too strong | ✅ Confirmed → **fixed** | Two explicit `NetworkMode`s — **PrivateOnly** (no NAT route at all; endpoint-only) and **ApprovedExternalConnectors** (NAT + proxy/allowlist/firewall); README/heading + compliance-doc wording corrected | `network.yaml` (Condition `AllowExternalEgress`); README §Network isolation |
| 7 | AgentCore targets lack ToolSchema (undeployable as written) | ✅ Confirmed → **labelled experimental** | Template banner + README/SUITE-STATUS now call portable the supported default and AgentCore **experimental** (each target needs a ToolSchema); no longer "interchangeable" | `agentcore-gateway.yaml`; README line 12; SUITE-STATUS |
| 8 | IdP is SAML, not auto SAML/OIDC; Part 11 "satisfies" too strong | ✅ Confirmed → **fixed (wording)** | README/BATTLECARD: audit "**designed to support** 21 CFR Part 11 … subject to validation"; SAML implemented / OIDC requires documented adaptation | README §8, line 8; `offerings/BATTLECARD.md` |
| 9 | Shared IAM broader than ideal (Bedrock `Resource:"*"`, multi-principal trust) | ✅ Confirmed → **improved** | Bedrock scoped to foundation-model + this guardrail ARN; trust narrowed to ECS/Lambda; added dedicated **AuditWriter / ApprovalService / WormExporter** roles + outputs (canonical golden path already per-function) | `security.yaml` |
| 10 | No deployment has validated the infra claims | ✅ Confirmed (acknowledged) | Cannot deploy from here. Clean-account acceptance test remains the gate; everything below cfn-lint + unit level is honestly marked "needs live" | this plan; SUITE-STATUS |
| 11 | CI lacks supply-chain + deploy testing | ✅ Confirmed → **authored** | New `supply-chain` CI job: gitleaks, bandit (SAST), pip-audit (SCA), checkov (IaC), terraform fmt/validate, CycloneDX SBOM, Trivy; provenance/cosign noted | `.github/workflows/ci.yml` |

**Still requires a live AWS account (P0 #10 + the live halves of #2, #6, #7):** federated reviewer login asserting `custom:hcls_role`; the reviewer **UI**; confirming PrivateOnly truly has no egress path and endpoints carry the traffic; completing AgentCore **ToolSchema** per target; and running the full clean-account acceptance test (deploy → federated auth → ingest → reject-bad-approval → accept-bound-approval → governed submit → immutable audit → reconcile → export → destroy). **Verification:** suite **518 green** one-command; all templates cfn-lint E-clean.
