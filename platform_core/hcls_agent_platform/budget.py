...
spend occurs — fail-closed on budget — plus one-time threshold alerts. This is the offline
analog of the gateway meter; in production AWS Budgets is a second account-level guardrail.
Ported from the Aegis platform (token_budget.py) so every AGP-conformant pack enforces the
same cost-control contract. No dependencies.

    preflight(estimated_tokens) -> BudgetDecision   # before spend; does not mutate
    commit(actual_tokens)       -> [newly-crossed alert thresholds]   # after spend
"""
...
class BudgetMeter:
    ...
    def preflight(self, estimated_tokens: int) -> BudgetDecision:
        """Decide whether a call estimated at `estimated_tokens` may proceed (no mutation)."""
