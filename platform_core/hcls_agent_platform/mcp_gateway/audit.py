...
approver where a human gate applied. Free-text args are PHI-masked before they
are written.

In production the sink is an append-only store (DynamoDB with a deny on
UpdateItem/DeleteItem, or QLDB) so the trail is tamper-evident by design — the
21 CFR Part 11 audit-trail control. Here it is an in-memory list plus optional
JSONL file so the model is testable.
"""
