"""
Provider-abstracted LLM factory for the HCLS agent suite.
...
One factory, two providers, two model tiers:
    ...
    PROVIDERS (env: LLM_PROVIDER)
      ...
      bedrock               ChatBedrockConverse via a VPC interface endpoint —
                            inference stays inside the customer's AWS account.
                            THIS is the configuration that makes the data-residency
                            story (no PHI/IP egress to external AI APIs) true, and
                            the model tier that Amazon Bedrock AgentCore Runtime uses.

    TIERS (role argument)
      narrative  Claude Sonnet — regulatory submission sections, CSR text, SAE
                 narratives, CAPA plans, MSL briefings: anything a regulator,
                 medical reviewer, or HCP reads.
      fast       Claude Haiku — high-volume triage, classification, MedDRA-coding
                 assist, TMF completeness checks where latency / unit cost dominate.
...
(PHI filters, denied topics, off-label-claim blocking, contextual grounding checks).
Guardrails are a deployment control — configure them in IaC (infra/cloudformation/
security, infra/terraform/modules/security) and reference the ID here.
...
def get_llm(role: Role = "narrative", temperature: float = 0.0, max_tokens: int = 4096) -> Any:
    ...
    if provider() == "bedrock":
        ...
        if guardrail_id:
            ...
            # Guardrails are a production control. In a production environment (or when
            # REQUIRE_BEDROCK_GUARDRAIL is set) refuse to start without one, rather than
            # silently running un-guardrailed inference on PHI/regulated content.
            # Dev/demo only warns.
            require = (
