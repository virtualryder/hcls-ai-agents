# Golden Path — 03 Clinical Trial Ops & TMF (one command)
...
```bash
cd infra/golden-path-03-clinical-trial-ops
./deploy.sh hcls-03-dev          # sam build && sam deploy
./smoke_test.sh hcls-03-dev      # start execution → approve human gate (bound, SoD) → assert SUCCEEDED
./destroy.sh hcls-03-dev         # sam delete (Retain tables remain)
```
