# Golden Path — 06 Clinical Protocol Design & Feasibility (one command)
...
```bash
cd infra/golden-path-06-protocol-design
./deploy.sh hcls-06-dev          # sam build && sam deploy
./smoke_test.sh hcls-06-dev      # start execution → approve human gate (bound, SoD) → assert SUCCEEDED
./destroy.sh hcls-06-dev         # sam delete (Retain tables remain)
```
