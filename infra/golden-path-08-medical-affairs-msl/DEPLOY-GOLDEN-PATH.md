# Golden Path — 08 Medical Affairs / MSL Copilot (one command)
...
```bash
cd infra/golden-path-08-medical-affairs-msl
./deploy.sh hcls-08-dev          # sam build && sam deploy
./smoke_test.sh hcls-08-dev      # start execution → approve human gate (bound, SoD) → assert SUCCEEDED
./destroy.sh hcls-08-dev         # sam delete (Retain tables remain)
```
