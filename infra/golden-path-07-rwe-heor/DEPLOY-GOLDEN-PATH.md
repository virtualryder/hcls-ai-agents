# Golden Path — 07 Real-World Evidence / HEOR (one command)
...
```bash
cd infra/golden-path-07-rwe-heor
./deploy.sh hcls-07-dev          # sam build && sam deploy
./smoke_test.sh hcls-07-dev      # start execution → approve human gate (bound, SoD) → assert SUCCEEDED
./destroy.sh hcls-07-dev         # sam delete (Retain tables remain)
```
