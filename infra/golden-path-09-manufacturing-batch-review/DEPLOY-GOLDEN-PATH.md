# Golden Path — 09 Manufacturing Batch-Review (one command)
...
```bash
cd infra/golden-path-09-manufacturing-batch-review
./deploy.sh hcls-09-dev          # sam build && sam deploy
./smoke_test.sh hcls-09-dev      # start execution → approve human gate (bound, SoD) → assert SUCCEEDED
./destroy.sh hcls-09-dev         # sam delete (Retain tables remain)
```
