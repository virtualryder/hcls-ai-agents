#!/usr/bin/env python3
"""Reviewer STAND-IN for the 09-manufacturing-batch-review smoke test (the deployed reviewer service is the next delivery step).
...
tool, args = "mes.write_disposition_draft", {"id": raw.get("case_id") or raw.get("batch_id") or raw.get("review_id") or "DRAFT"}
token = approvals.mint_approval_token(requestor=requestor, agent_id=AGENT, tool=tool, args=args, approver=approver)
out = {"approved": True, "decision": "APPROVE", "reviewer": {"sub": approver, "custom:hcls_role": "QA_RELEASE"}, "token": token}
