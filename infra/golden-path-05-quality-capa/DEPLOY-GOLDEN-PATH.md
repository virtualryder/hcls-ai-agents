# Golden Path — 05 Quality / CAPA & Complaints (one command)
...
```bash
cd infra/golden-path-05-quality-capa
./deploy.sh hcls-05-dev          # sam build && sam deploy
./smoke_test.sh hcls-05-dev      # start execution → approve human gate (bound, SoD) → assert SUCCEEDED
./destroy.sh hcls-05-dev         # sam delete (Retain tables remain)
```
