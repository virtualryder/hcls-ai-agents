# Golden Path — 02 Pharmacovigilance (one command)
...
```bash
cd infra/golden-path-02-pharmacovigilance
./deploy.sh hcls-02-dev          # sam build && sam deploy
./smoke_test.sh hcls-02-dev      # start execution → approve human gate (bound, SoD) → assert SUCCEEDED
./destroy.sh hcls-02-dev         # sam delete (Retain tables remain)
```
