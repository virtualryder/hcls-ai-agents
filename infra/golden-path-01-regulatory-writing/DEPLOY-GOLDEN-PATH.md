# Golden Path — 01 Regulatory Writing & Intelligence (one command)
...
```bash
cd infra/golden-path-01-regulatory-writing
./deploy.sh hcls-01-dev          # sam build && sam deploy
./smoke_test.sh hcls-01-dev      # start execution → approve human gate (bound, SoD) → assert SUCCEEDED
./destroy.sh hcls-01-dev         # sam delete (Retain tables remain)
```
