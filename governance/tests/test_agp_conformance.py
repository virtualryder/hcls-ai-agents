...
def test_declares_agp_1_0():
    pkg = importlib.import_module("hcls_agent_platform")
    assert getattr(pkg, "AEGIS_GOVERNANCE_PATTERN_VERSION", None) == "1.0"


def test_token_budget_module_present():
    m = importlib.import_module("hcls_agent_platform.budget")
    assert hasattr(m, "BudgetMeter")


def test_core_control_modules_import():
    for mod in ("hcls_agent_platform.mcp_gateway.gateway", "hcls_agent_platform.mcp_gateway.policy",
                "hcls_agent_platform.mcp_gateway.audit"):
