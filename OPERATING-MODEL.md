# Operating Model — HCLS / Life Sciences
...
| 2 | **Who approves consequential actions?** | The framework-enforced human-approval gate (bound, single-use, SoD tokens) — the *mechanism* | Names the approver role and staffs it (**PV Medical Reviewer / QA (GxP)**); the human makes every consequential decision | Accel: R (mechanism) · Customer: A (decision) |
...
| 6 | **Who answers audit requests?** | Append-only + WORM audit, control matrices (NIST/OWASP), the assurance packet, this operating model | Produces the evidence to the auditor; owns the FDA / 21 CFR Part 11 / GxP inspection response (**Compliance / GRC**) | Accel: C · Customer: R/A |
...
3. Business reviewer (PV Medical Reviewer / QA (GxP)) and the AI/ML owner approve through the customer's change process.
...
- [ ] Approver role staffed (PV Medical Reviewer / QA (GxP)); SoD enforced (`STRICT_APPROVAL=1` / bound approvals only).
...
- [ ] GRC owner for FDA / 21 CFR Part 11 / GxP inspection; audit-evidence export tested from the append-only trail.
